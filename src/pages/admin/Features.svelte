...
<script>
  ...
  import FeatureEditor from '@/components/FeatureEditor.svelte';
  ...
  const deleteFeature = (slug, title) => {
    if (!confirm(`Tem certeza que deseja excluir o atributo "${title}".`))
      return;
  ...
  <div class="flex flex-col mt-6 lg:flex-row lg:items-end lg:justify-between">
    ...
    >
      <div class="flex items-center justify-start w-full h-10 px-3 text-gray-100 rounded bg-main-500 lg:justify-between lg:w-40 hover:bg-main-600 active:bg-main-400">
        ...
        <span class="w-full font-medium text-center">Novo Atributo</span>
      </div>
    </button>
    ...
    <table class="flex flex-col space-y-3 text-sm text-gray-700">
      <thead class="sticky text-gray-700 bg-white rounded-lg shadow-lg top-[-15px] uppercase">
        <tr class="flex justify-between">
          <th class="w-20 p-3 ml-3 text-left md:w-full">Nome</th>
          <th class="flex-shrink-0 w-20 py-3 text-left md:ml-auto md:p-3 md:text-center md:w-40">Ações</th>
        </tr>
      </thead>
      <tbody class="flex flex-col space-y-3 flex-rol">
        ...
        >
          ...
          {#if filterSearch(features, currentSearchTerm).length === 0}
          ...
          {#each filterSearch(features, currentSearchTerm) as feature, i (i)}
            <tr class="flex w-full bg-white rounded-lg shadow-md space-between">
              <td class="flex items-center flex-shrink-0 w-20 col-span-3 p-3 md:flex-shrink md:w-full">
                <span class="ml-3 text-sm font-semibold">{feature.name}</span>
              </td>
              <td class="flex items-center justify-center p-3 ml-auto space-x-3 w-30 md:flex-shrink-0 md:w-40">
                <button
                  title="Editar"
                  class="flex items-center justify-center p-2 text-blue-500 rounded hover:bg-blue-200 active:bg-blue-100"
                  on:click={() => showFeatureEditor(false, feature.id)}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <button
                  ...
                  on:click={() => deleteFeature(feature.id, feature.name)}
                >
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
